...
function getDraftingArtifacts(messages: Message[]) {
  ...
  const transformed = messages.map((message) => {
    ...
    latestDraftText = message.content;
    return {
      ...message,
...
export default function App() {
  ...
  const [draftingSettings, setDraftingSettings] = useState<DraftingSettings | null>(null);
  ...
  const [selectedConversationIds, setSelectedConversationIds] = useState<{
    ...
    drafting_studio: string | null;
  }>({
    judgment: null,
    drafting_studio: null,
  });
  ...
  const activeDraftAnswerTypeRef = useRef<"drafting_questions" | "drafting_draft" | null>(null);
  ...
  useEffect(() => {
    ...
    if (mode === "drafting_document") {
      setActiveView("drafting_document");
    }
  }, []);

  const workspaceView: WorkspaceView =
    activeView === "drafting_document" ? "drafting_document" : "chat";
  const currentChatMode = getChatModeForView(workspaceView);
  const isDraftingMode = workspaceView === "drafting_document";
  const activeConversationId = selectedConversationIds[currentChatMode] ?? null;
  ...
  const composerPlaceholder = isDraftingMode
    ? "Describe the notice, agreement, reply, clause, or legal draft you want to create..."
  ...
  const setConversationIdForMode = (
  ...
  const applyMessagesForWorkspace = (
    ...
    if (view === "drafting_document") {
      const { messages: transformed, latestDraftText } =
        getDraftingArtifacts(nextMessages);

      if (conversationId && latestDraftText.trim()) {
        setDraftContentsByConversation((prev) => ({
          ...prev,
          [conversationId]: latestDraftText,
        }));
      }
    ...
    setMessages(nextMessages.length ? nextMessages : [makeWorkspaceStarterMessage(view)]);
  };
  ...
  const loadConversationMessages = async (
    ...
    try {
      ...
      applyMessagesForWorkspace(view, conversationId, mapped);
      return response;
    } finally {
  ...
  const handleSelectConversation = async (conversationId: string) => {
    if (loading) return;
    setConversationIdForMode(currentChatMode, conversationId);
    await loadConversationMessages(conversationId, workspaceView);
  };

  const handleNewConversation = async (view: WorkspaceView = workspaceView) => {
    const chatMode = getChatModeForView(view);
    ...
    setActiveView(view);
    setConversationIdForMode(chatMode, newConversation.id);
    setMessages([makeWorkspaceStarterMessage(view)]);
    setInput("");
    focusComposer();
  };
  ...
  const handleShareChat = async () => {
    ...
    if (isWorkspaceView(activeView) && activeConversationId) {
  ...
  const handleQuickReply = (value: string) => {
    setInput((prev) => {
      if (!prev.trim()) return value.slice(0, MAX_INPUT_LENGTH);
  ...
  useEffect(() => {
    if (!user || !isWorkspaceView(activeView)) return;
    ...
    (async () => {
      ...
      try {
        const conversationResponse = await conversationService.list(currentChatMode);
        ...
        const preferredConversationId =
          ...
          (selectedConversationIds[currentChatMode] &&
            conversationResponse.conversations.find(
              (item) => item.id === selectedConversationIds[currentChatMode]
            )?.id) ||
          conversationResponse.conversations[0]?.id ||
          null;

        setConversationIdForMode(currentChatMode, preferredConversationId);
        ...
        if (preferredConversationId) {
          ...
          setMessages([makeWorkspaceStarterMessage(activeView)]);
        }
      } catch (error) {
        if (!cancelled) {
          console.error(error);
          setConversations([]);
          setConversationIdForMode(currentChatMode, null);
          setMessages([makeWorkspaceStarterMessage(activeView)]);
        }
      } finally {
        if (!cancelled) {
  ...
  const sendCurrentMessage = async () => {
    if (!canSend || !isWorkspaceView(activeView)) return;
    ...
    setMessages((prev) => {
      const starterContent = makeWorkspaceStarterMessage(activeView).content;
      const base =
        prev.length === 1 &&
      ...
      await streamChat(
        ...
        (event) => {
          ...
          if (streamedConversationId) {
            setConversationIdForMode(currentChatMode, streamedConversationId);
          }
        ...
        <div className="mx-auto min-h-screen lg:flex lg:h-screen">
          ...
          <main className="flex min-w-0 flex-1 flex-col lg:h-screen">
            ...
            <div className="flex min-h-0 flex-1 flex-col">
              ...
              {isWorkspaceView(activeView) && !isDraftingMode && (
                <WorkspacePane
                  messages={messages}
            ...
            {isDraftingMode && (
              <div className="flex min-h-0 flex-1 overflow-hidden bg-[#f6f4fb]">
                <div className="flex h-full min-h-0 w-full max-w-[430px] shrink-0 border-r border-slate-200 bg-[#faf8ff]">
                  <WorkspacePane
                    compactMode
                    messages={messages}
                    messagesLoading={messagesLoading}
                ...
                {currentDraftText.trim() ? (
                  ...
                  <div className="flex min-h-0 flex-1 items-center justify-center p-8">
                    <div className="max-w-[560px] rounded-[32px] border border-[#e7e0ff] bg-white px-8 py-10 shadow-sm">
                      <div className="text-[11px] font-semibold uppercase tracking-[0.18em] text-[#7b6ee6]">
                        Drafting workspace
                      </div>
                      <div className="mt-3 text-2xl font-semibold text-slate-900">
                        Your draft will open here
                      </div>
                      <div className="mt-4 text-sm leading-7 text-slate-500">
                        Use the chat on the left to describe the document or answer the AI’s
                        questions. The editor will appear only after the first real draft is generated.
                      </div>
                    </div>
                  </div>
                )}
